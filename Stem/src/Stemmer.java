...
class Stemmer {
	 ...
	 * (j < 500) j++; ch = in.read(); if (!Character.isLetter((char) ch)) { to
	 * test add(char ch) for (int c = 0; c < j; c++) s.add(w[c]);
	 * 
	 ...
	 * 
	 * to test getResultBuffer(), getResultLength() : u = new
	 * String(s.getResultBuffer(), 0, s.getResultLength());
	 * 
