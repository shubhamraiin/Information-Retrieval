...
splitting words on dashes, removing fullstops and combining the acronyms etc.
After token have been recognized this class calls the Porter's implementation of Stemmer and stems the words.  
...
public class Tokenizer {
	...
	public Tokenizer(String pathDirectory) {
   ...
   //Parsing the files and building the list
	...
	private void processToken(String token) {
		...
		if (token.endsWith("'s")) {
			...
			stem.add(token.toCharArray(), token.toCharArray().length);
			stem.stem();
			token = stem.toString();
